...
set<Vertex> bfs(const Vertex& start, const UnweightedGraph<Vertex>* graph) {
	...
	while (!pending.empty()){
		...
		if (visited.find(v) == visited.end()){
			visited.insert(v);
			set<Vertex> connected =	graph->getConnectionFrom(v);
			for (itr = connected.begin();
                itr != connected.end(); itr++) {
				pending.push((*itr));
			}
...
template <class Vertex>
int consulta(const Vertex &start, UnweightedGraph<Vertex> &graph, int NMP) {
    set<Vertex> pending;
    ...
    while (!q.empty()){
        ...
        q.pop();
        Vertex v = vp.first;
        int visited = vp.second;
        if (pending.find(v)==pending.end()){
            pending.insert(v);
            if (visited < NMP){
                for (Vertex u : graph.getConnectionFrom(v)){
                    q.push({u, visited + 1});
                }
            }
        }
    }
    return bfs(start,&graph).size()-pending.size();
}
