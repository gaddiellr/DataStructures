...
string BST<T>::byLevel() const {
	...
	if (!empty()) {
        Node<T>* parent;
        parent=root;
        aux<<to_string(parent->value)<<" ";
        vector<Node<T>*> vParent;
        Node<T>* x;
        int i=0;
        string str;
        while (parent->left!=NULL||parent->right!=NULL){
            if (i>0){
                x=vParent[(i-1)/2]; // i=1:[0] , i=2:[0] , i=3:[1] , i=4:[1] , i=5:[2] , i=6:[2] ...
            }
            if (i>0&&i%2==1){
                parent=x->left;
            }
            ...
            if (parent->left!=NULL){
                str=to_string(parent->left->value);
                aux<<str<<" ";
            }
            if (parent->right!=NULL){
                str=to_string(parent->right->value);
                aux<<str<<" ";
            }
            i++;
        }
